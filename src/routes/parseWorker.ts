...
async function parse(string: string): Promise<dataPackage> {
	...
	const songs = tracksDict.descendantsOfType("obj").slice(1).map((item): song => {
		let name: string | undefined, artist: string | undefined, album: string | undefined, genre: string | undefined;
		let time: number | undefined, playCount: number | undefined, skipCount: number | undefined, rating: number | undefined;
		...
		while (cursor.gotoNextSibling()) {
			if (cursor.nodeType == "item") {
				...
				if (dataNode == null) { throw "no data node in item" }
				switch (keyName) {
					case "Name":
						name = getAndParseKeyValue<string>(dataNode); break;
					case "Artist":
						artist = getAndParseKeyValue<string>(dataNode); break;
					case "Album":
						album = getAndParseKeyValue<string>(dataNode); break;
					case "Genre":
						genre = getAndParseKeyValue<string>(dataNode); break;
					case "Total Time":
						time = getAndParseKeyValue<number>(dataNode); break;
					case "Play Count":
						playCount = getAndParseKeyValue<number>(dataNode); break;
					case "Skip Count":
						skipCount = getAndParseKeyValue<number>(dataNode); break;
					case "Rating":
						rating = getAndParseKeyValue<number>(dataNode); break;
				}
			}
		}
		return { name: name, artist: artist, album: album, genre: genre, time: time, playCount: playCount, skipCount: skipCount, rating: rating }
	});
