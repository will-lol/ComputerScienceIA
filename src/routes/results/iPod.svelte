...
<div
	class="mx-auto aspect-ipod font-ipod grid p-4 bg-gradient-to-tr from-[#eceef0] to-[#fbfdfd] from-[47%] to-[53%] w-72 rounded-xl border-solid border-stone-300 border-2 shadow-lg"
>
	<div class="rounded text-xs w-full h-48 bg-[#f0f6fb] border-[3px] border-black border-solid">
		<div
			class="relative w-full px-1 h-5 text-sm text-center flex justify-between items-center border-b border-solid border-gray-400 bg-gradient-to-b from-[#f0f0f0] to-[#bfccd6]"
		>
			<div>
				<svg width="15" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path
						d="M27.5 14.634a1 1 0 0 1 0 1.732L7.25 28.057a1 1 0 0 1-1.5-.866V3.81a1 1 0 0 1 1.5-.866L27.5 14.634Z"
						fill="url(#a)"
					/>
					<path
						d="M27 15.5L6.75 27.1913L6.75 3.80866L27 15.5Z"
						stroke="black"
						stroke-opacity="0.2"
						stroke-width="3"
					/>
					<defs>
						<radialGradient
							...
							gradientTransform="rotate(180 6.75 7.75) scale(15.5)"
						>
			...
			<div>
				<svg width="25" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path
						d="M26 10H3v11h23v-3h2v-5h-2v-3Z"
						fill="url(#b)"
						stroke="black"
						stroke-opacity=".53"
						stroke-width="2"
		...
		<div class="grid grid-rows-6 gap-1 m-2">
			...
			<div class="h-full flex items-end">
				<div class="h-4 w-full bg-progress-empty shadow">
					<div class="h-full bg-progress shadow w-2/3" />
				</div>
			</div>
			<div class="w-full h-full flex content-between items-center justify-between">
				<div>
					{Math.trunc((minutesMinutes * 2) / 3)}:{padNum(Math.trunc((minutesSeconds * 2) / 3), 2)}
				</div>
				<div>
					-{Math.trunc((minutesMinutes * 1) / 3)}:{padNum(Math.trunc((minutesSeconds * 1) / 3), 2)}
				</div>
			</div>
		</div>
	</div>
	<div class="w-full">
		<div
			class="bg-[#e3e8e9] aspect-square relative mx-auto w-44 rounded-full border border-solid border-stone-300"
		>
			...
			<div
				class="w-16 border border-solid border-gray-300 rounded-full aspect-square bg-[#f8fafb] absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2"
			/>
		</div>
	</div>
</div>
