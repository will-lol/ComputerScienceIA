...
export async function createUser(username: string) {
	const output = await client.execute({
		sql: 'insert into UserTable (githubUsername) values (?) returning id',
		args: [username]
	});
...
export async function uploadData(username: string, data: dataPackage, date: Date) {
	...
	const dataAlreadyExists: boolean = await client
		.execute({ sql: 'select * from DataTable where hash = ?', args: [dataHash] })
	...
	await client.execute({
		sql: 'insert into DataTable (s3key, username, hash, date) values (?, ?, ?, ?)',
		args: [uuid, username, dataHash, date.valueOf()]
	});
}

export async function retreiveFromDB(username: string): Promise<retrieveAllDataApi> {
	const dbResults = await client.execute({
		sql: 'select s3key,id,date from DataTable where username = ?',
		args: [username]
	});
	return dbResults.rows.map((el) => {
		return { s3Key: el[0] as string, id: el[1] as number, date: new Date(el[2] as number) };
	});
}
